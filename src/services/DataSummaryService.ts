...
export class DataSummaryService {
    ...
    private updateSection(section: GroupEmailSummary, messages: Array<[Email, JunkEvaluation]>): void {
        messages.forEach(([email, evaluation]) => {
            ...
            const timestamps = section.details[reason].summary[senderEmail].timestamps;
            if (!timestamps.includes(email.receivedDateTime)) {
                timestamps.push(email.receivedDateTime);
            }

            section.details[reason].summary[senderEmail].total = timestamps.length;

            section.details[reason].total = Object.values(section.details[reason].summary)
                .reduce((total, sender) => total + sender.timestamps.length, 0);

            section.total = Object.values(section.details)
                .reduce((total, reasonData) => total + reasonData.total, 0);
        });
    }

    public recordDeletedMessages(messages: Array<[Email, JunkEvaluation]>): void {
        this.updateSection(this.report.deleted, messages);
    }

    public recordIgnoredMessages(messages: Array<[Email, JunkEvaluation]>): void {
